...
**🚨 This is the project behind my "YOLO object detection for robotics" experience — with real numbers, not hypotheticals.**
...
- **Real-time pipeline** — camera → YOLO (CUDA) → annotated overlay streamed via ffplay
...
├── doc.tex               # full technical report (Spanish) — architecture, ROI, ISO 27001/ITIL 4
├── people_log.json       # runtime activity log (gitignored)
├── runs/                 # Ultralytics training runs (gitignored)
└── scripts/
    └── download_weights.sh  # fetch pretrained weights (COCO.pt, yolo11m.pt, etc.)
```

## 📜 Documentation

The full technical report (`doc.tex`) covers:
...
The original notebook contained a hardcoded Roboflow API key — it has been removed and replaced with an environment variable. **Rotate your Roboflow key** if you ever committed this notebook with the key present.
